...
public class BookmarkDB extends SQLiteOpenHelper
{
	...
	static final String DB_KEY_BOOKMARK_SCREEN_ORIENTATION = "screen_orientation";
	static final String DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK = "auto_reconnect_on_kick";
	...
	{
		super.onOpen(db);
		// 检查必要的列是否存在，如果不存在就添加（兼容旧数据库）
		try
		{
			List<String> columns = GetColumns(db, DB_TABLE_BOOKMARK);
			if (columns != null)
			{
				if (!columns.contains(DB_KEY_BOOKMARK_SCREEN_ORIENTATION))
				{
					Log.d("BookmarkDB", "onOpen: Adding missing screen_orientation column to " + DB_TABLE_BOOKMARK);
					db.execSQL("ALTER TABLE " + DB_TABLE_BOOKMARK + " ADD COLUMN " + 
					           DB_KEY_BOOKMARK_SCREEN_ORIENTATION + " INTEGER DEFAULT 0");
				}
				
				if (!columns.contains(DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK))
				{
					Log.d("BookmarkDB", "onOpen: Adding missing auto_reconnect_on_kick column to " + DB_TABLE_BOOKMARK);
					db.execSQL("ALTER TABLE " + DB_TABLE_BOOKMARK + " ADD COLUMN " + 
					           DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK + " INTEGER DEFAULT 0");
				}
			}
		}
		catch (Exception e)
		{
			Log.e("BookmarkDB", "onOpen: Error checking/adding screen_orientation column: " + e.getMessage(), e);
		}
	}
	...
	@Override public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion)
	{
		switch (oldVersion)
		{
			...
			case 10:
			case 11:
				// 从版本10/11升级到12：添加 auto_reconnect_on_kick 列
				if (newVersion >= 11)
				{
					try
					{
						// 检查并添加 screen_orientation 列（如果从版本10升级）
						List<String> columns = GetColumns(db, DB_TABLE_BOOKMARK);
						if (columns != null && !columns.contains(DB_KEY_BOOKMARK_SCREEN_ORIENTATION))
						{
							Log.d("BookmarkDB", "Adding screen_orientation column to " + DB_TABLE_BOOKMARK);
							db.execSQL("ALTER TABLE " + DB_TABLE_BOOKMARK + " ADD COLUMN " + 
							           DB_KEY_BOOKMARK_SCREEN_ORIENTATION + " INTEGER DEFAULT 0");
						}
						
						// 添加 auto_reconnect_on_kick 列（版本12）
						if (newVersion >= 12)
						{
							columns = GetColumns(db, DB_TABLE_BOOKMARK);
							if (columns != null && !columns.contains(DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK))
							{
								Log.d("BookmarkDB", "Adding auto_reconnect_on_kick column to " + DB_TABLE_BOOKMARK);
								db.execSQL("ALTER TABLE " + DB_TABLE_BOOKMARK + " ADD COLUMN " + 
								           DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK + " INTEGER DEFAULT 0");
							}
						}
					...
					{
						Log.e("BookmarkDB", "Error adding columns: " + e.getMessage(), e);
						// 如果添加列失败，回退到完整的升级流程
						upgradeDB(db);
					}
				}
