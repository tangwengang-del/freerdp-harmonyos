...
public class KeyboardMapper
{
	...
	private void resetModifierKeysAfterInput(boolean force)
	{
		if (shiftPressed && (!isShiftLocked || force))
		{
			...
			shiftPressed = false;
			if (listener != null) {
				listener.onModifierKeyReleased(VK_LSHIFT);
			}
		}
		if (ctrlPressed && (!isCtrlLocked || force))
		{
			...
			ctrlPressed = false;
			if (listener != null) {
				listener.onModifierKeyReleased(VK_LCONTROL);
			}
		}
		if (altPressed && (!isAltLocked || force))
		{
			...
			isAltLocked = false;
			if (listener != null) {
				listener.onModifierKeyReleased(VK_LMENU);
			}
		}
		if (winPressed && (!isWinLocked || force))
		{
			...
			winPressed = false;
			if (listener != null) {
				listener.onModifierKeyReleased(VK_LWIN);
			}
		}
	...
	// Public methods to set modifier lock state from external (e.g., SessionActivity's ALT button)
	public void setAltLocked(boolean locked) {
		isAltLocked = locked;
		altPressed = locked;
		if (listener != null) {
			listener.processVirtualKey(VK_LMENU, locked);
			listener.modifiersChanged();
		}
	}
	...
	public interface KeyProcessingListener {
		...
		// NEW: Callback when modifier key state changes (for UI synchronization)
		void onModifierKeyReleased(int virtualKeyCode);
	}
}
