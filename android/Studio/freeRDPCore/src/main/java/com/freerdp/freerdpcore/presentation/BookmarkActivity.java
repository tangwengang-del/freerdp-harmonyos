...
public class BookmarkActivity extends PreferenceActivity implements OnSharedPreferenceChangeListener
{
	...
	private void initAdvancedSettings(SharedPreferences sharedPreferences)
	{
		...
		advancedSettingsChanged(sharedPreferences, "bookmark.screen_orientation");
	}
	...
	private void advancedSettingsChanged(SharedPreferences sharedPreferences, String key)
	{
		...
		else if (key.equals("bookmark.screen_orientation"))
		{
			// 屏幕方向选项 - 确保有默认值
			com.freerdp.freerdpcore.utils.IntListPreference listPreference = 
				(com.freerdp.freerdpcore.utils.IntListPreference)findPreference(key);
			if (listPreference != null)
			{
				int orientation = sharedPreferences.getInt(key, 0);
				// 设置值，让IntListPreference知道当前选择
				listPreference.setValue(String.valueOf(orientation));
			}
		}
				    ...
				    .setPositiveButton(
				        ...
				        new DialogInterface.OnClickListener() {
					        @Override public void onClick(DialogInterface dialog, int which)
					        {
						        ...
						        int screenOrientationBefore = bookmark.getAdvancedSettings().getScreenOrientation();
						        int screenOrientationInPrefs = prefs.getInt("bookmark.screen_orientation", -999);
						        Log.d("BookmarkActivity", "Before readFromSharedPreferences: screenOrientation=" + screenOrientationBefore + ", in SharedPreferences=" + screenOrientationInPrefs);
						        
						        bookmark.readFromSharedPreferences(prefs);
						        
						        int screenOrientationAfter = bookmark.getAdvancedSettings().getScreenOrientation();
						        Log.d("BookmarkActivity", "After readFromSharedPreferences: screenOrientation=" + screenOrientationAfter);
