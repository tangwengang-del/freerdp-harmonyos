...
public abstract class BookmarkBaseGateway
{
	...
	int screenOrientation = bookmark.getAdvancedSettings().getScreenOrientation();
	android.util.Log.d("BookmarkBaseGateway", "Saving screen_orientation=" + screenOrientation + " for bookmark id=" + bookmark.getId() + ", label=" + bookmark.getLabel());
	values.put(BookmarkDB.DB_KEY_BOOKMARK_SCREEN_ORIENTATION, screenOrientation);
	
	boolean autoReconnectOnKick = bookmark.getAdvancedSettings().getAutoReconnectOnKick();
	android.util.Log.d("BookmarkBaseGateway", "Saving auto_reconnect_on_kick=" + autoReconnectOnKick + " for bookmark id=" + bookmark.getId() + ", label=" + bookmark.getLabel());
	values.put(BookmarkDB.DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK, autoReconnectOnKick ? 1 : 0);
	...
	// update bookmark
	int updateCount = db.update(getBookmarkTableName(), values,
	                         BookmarkDB.ID + " = " + bookmark.getId(), null);
	boolean res = (updateCount == 1);
	android.util.Log.d("BookmarkBaseGateway", "UPDATE: db.update() returned " + updateCount + " rows affected for bookmark id=" + bookmark.getId());
	...
	private void addBookmarkColumns(ArrayList<String> columns)
	{
		...
		columns.add(BookmarkDB.DB_KEY_BOOKMARK_SCREEN_ORIENTATION);
		columns.add(BookmarkDB.DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK);
	...
	// 新字段：屏幕方向（带默认值保护，避免旧数据库崩溃）
	int orientationIdx = cursor.getColumnIndex(BookmarkDB.DB_KEY_BOOKMARK_SCREEN_ORIENTATION);
	android.util.Log.d("BookmarkBaseGateway", "READ: Column index for screen_orientation=" + orientationIdx + ", column name=" + BookmarkDB.DB_KEY_BOOKMARK_SCREEN_ORIENTATION);
	int screenOrientation = orientationIdx >= 0 ? cursor.getInt(orientationIdx) : 0;
	if (orientationIdx >= 0)
	{
		android.util.Log.d("BookmarkBaseGateway", "READ: Raw value from cursor=" + cursor.getInt(orientationIdx));
	}
	else
	{
		android.util.Log.w("BookmarkBaseGateway", "READ: Column not found, using default value 0");
	}
	android.util.Log.d("BookmarkBaseGateway", "READ: Loading screen_orientation=" + screenOrientation + " for bookmark id=" + bookmark.getId() + ", label=" + bookmark.getLabel());
	bookmark.getAdvancedSettings().setScreenOrientation(screenOrientation);
	
	// 新字段：被踢出后自动重连（带默认值保护）
	int autoReconnectIdx = cursor.getColumnIndex(BookmarkDB.DB_KEY_BOOKMARK_AUTO_RECONNECT_ON_KICK);
	boolean autoReconnectOnKick = autoReconnectIdx >= 0 ? cursor.getInt(autoReconnectIdx) == 1 : false;
	android.util.Log.d("BookmarkBaseGateway", "READ: Loading auto_reconnect_on_kick=" + autoReconnectOnKick + " (column index=" + autoReconnectIdx + ") for bookmark id=" + bookmark.getId() + ", label=" + bookmark.getLabel());
	bookmark.getAdvancedSettings().setAutoReconnectOnKick(autoReconnectOnKick);
