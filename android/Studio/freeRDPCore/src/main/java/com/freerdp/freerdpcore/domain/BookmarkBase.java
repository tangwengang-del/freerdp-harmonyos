...
public class BookmarkBase implements Parcelable, Cloneable
{
	...
	editor.putInt("bookmark.screen_orientation", advancedSettings.getScreenOrientation());
	editor.putBoolean("bookmark.auto_reconnect_on_kick", advancedSettings.getAutoReconnectOnKick());
	Log.d("BookmarkBase", "Write auto_reconnect_on_kick to SharedPreferences: " + advancedSettings.getAutoReconnectOnKick());
	...
	int screenOrientation = sharedPrefs.getInt("bookmark.screen_orientation", 0);
	android.util.Log.d("BookmarkBase", "Reading screen_orientation from SharedPreferences: " + screenOrientation);
	advancedSettings.setScreenOrientation(screenOrientation);
	boolean autoReconnectOnKick = sharedPrefs.getBoolean("bookmark.auto_reconnect_on_kick", false);
	android.util.Log.d("BookmarkBase", "Reading auto_reconnect_on_kick from SharedPreferences: " + autoReconnectOnKick);
	advancedSettings.setAutoReconnectOnKick(autoReconnectOnKick);
	...
	private int screenOrientation;  // 0=自动, 1=横屏, 2=竖屏
	private boolean autoReconnectOnKick;  // 被踢出后是否自动重连
	...
	autoReconnectOnKick = parcel.readInt() == 1;
}
	...
	remoteProgram = "";
	workDir = "";
	screenOrientation = 0;  // 默认自动
	autoReconnectOnKick = false;  // 默认不重连
}
	...
	public boolean getAutoReconnectOnKick()
	{
		return autoReconnectOnKick;
	}

	public void setAutoReconnectOnKick(boolean autoReconnectOnKick)
	{
		this.autoReconnectOnKick = autoReconnectOnKick;
	}
	...
	out.writeInt(autoReconnectOnKick ? 1 : 0);
}
}
}
