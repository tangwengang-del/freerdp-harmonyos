...
/* Cursor type definitions for Java layer callback */
...
#define CURSOR_TYPE_SIZE_NS 4      // 上下双箭头（调整行高）
#define CURSOR_TYPE_SIZE_WE 5      // 左右双箭头（调整列宽）
...
static BOOL android_end_paint(rdpContext* context)
{
	...
	freerdp_callback("OnGraphicsUpdate", "(JIIII)V", (jlong)(uintptr_t)context->instance, x1, y1, x2 - x1,
	                 y2 - y1);
...
static BOOL android_desktop_resize(rdpContext* context)
{
	...
	freerdp_callback("OnGraphicsResize", "(JIII)V", (jlong)(uintptr_t)context->instance,
...
static BOOL android_pre_connect(freerdp* instance)
{
	...
	freerdp_callback("OnPreConnect", "(J)V", (jlong)(uintptr_t)instance);
	return TRUE;
}
...
static BOOL android_Pointer_Set(rdpContext* context, rdpPointer* pointer)
{
	...
	if (instance)
	{
		freerdp_callback("OnCursorTypeChanged", "(JI)V", 
		                 (jlong)(uintptr_t)instance, 
		                 (jint)cursorType);
	}
...
{
	WINPR_ASSERT(context);
	WLog_INFO(TAG, "[POINTER] SetPosition: x=%u, y=%u", x, y);
	return TRUE;
}
...
static BOOL android_Pointer_SetNull(rdpContext* context)
{
	...
	WLog_DBG(TAG, "Pointer_SetNull");
	
	// 通知Java层：光标隐藏（使用正确的方法名）
	freerdp* instance = context->instance;
	if (instance)
	{
		freerdp_callback("OnCursorTypeChanged", "(JI)V", 
		                 (jlong)(uintptr_t)instance, 
		                 (jint)CURSOR_TYPE_UNKNOWN);
	}
...
static BOOL android_Pointer_SetDefault(rdpContext* context)
{
	...
	WLog_DBG(TAG, "Pointer_SetDefault");
	
	// 通知Java层：恢复默认光标（使用正确的方法名）
	freerdp* instance = context->instance;
	if (instance)
	{
		freerdp_callback("OnCursorTypeChanged", "(JI)V", 
		                 (jlong)(uintptr_t)instance, 
		                 (jint)CURSOR_TYPE_DEFAULT);
	}
...
static BOOL android_post_connect(freerdp* instance)
{
	...
	freerdp_callback("OnSettingsChanged", "(JIII)V", (jlong)(uintptr_t)instance,
	                 freerdp_settings_get_uint32(settings, FreeRDP_DesktopWidth),
	                 freerdp_settings_get_uint32(settings, FreeRDP_DesktopHeight),
	                 freerdp_settings_get_uint32(settings, FreeRDP_ColorDepth));
	freerdp_callback("OnConnectionSuccess", "(J)V", (jlong)(uintptr_t)instance);
	return TRUE;
}

static void android_post_disconnect(freerdp* instance)
{
	freerdp_callback("OnDisconnecting", "(J)V", (jlong)(uintptr_t)instance);
	gdi_free(instance);
}
                                     ...
                                     char** domain, const char* cb_name)
{
	...
	res = freerdp_callback_bool_result(cb_name,
	                                   ...
	                                   (jlong)(uintptr_t)instance, jstr1, jstr2, jstr3);
                                           ...
                                           const char* issuer, const char* fingerprint, DWORD flags)
{
	...
	jint res = freerdp_callback_int_result("OnVerifyCertificateEx",
	                                       ...
	                                       (jlong)(uintptr_t)instance, jstr0, (jlong)port, jstr1, jstr2, jstr3,
	                                       jstr4, (jlong)flags);
                                                   ...
                                                   const char* old_fingerprint, DWORD flags)
{
	...
	jint res =
	    freerdp_callback_int_result("OnVerifyChangedCertificateEx",
	                                ...
	                                (jlong)(uintptr_t)instance, jhost, (jlong)port, jstr0, jstr1, jstr2, jstr3,
...
fail:
	...
	if (status == CHANNEL_RC_OK)
		freerdp_callback("OnDisconnected", "(J)V", (jlong)(uintptr_t)instance);
	else
		freerdp_callback("OnConnectionFailure", "(J)V", (jlong)(uintptr_t)instance);
    ...
    JNIEnv* env, jclass cls, jobject context)
{
	...
	return (jlong)(uintptr_t)ctx->instance;
}
    ...
    JNIEnv* env, jclass cls, jlong instance, jint x, jint y, jint flags)
{
	...
	WLog_INFO(TAG, "[JNI-C] send_cursor START: inst=%p (0x%llX), x=%d, y=%d, flags=0x%04X, ptr_size=%zu, arch=%s", 
	          (void*)inst, (unsigned long long)instance, (int)x, (int)y, (int)flags, 
	          sizeof(void*), sizeof(void*) == 8 ? "64bit" : "32bit");
	
	if (!inst) {
		WLog_ERR(TAG, "[JNI-C] instance NULL!");
		return JNI_FALSE;
	}
	
	if (!inst->context) {
		WLog_ERR(TAG, "[JNI-C] instance->context NULL!");
		return JNI_FALSE;
	}
	
	event = (ANDROID_EVENT*)android_event_cursor_new((int)flags, (int)x, (int)y);

	if (!event) {
		WLog_ERR(TAG, "[JNI-C] event create FAIL");
		return JNI_FALSE;
	}
	
	WLog_INFO(TAG, "[JNI-C] event created: %p", (void*)event);

	if (!android_push_event(inst, event))
	{
		WLog_ERR(TAG, "[JNI-C] push event FAIL");
		android_event_free(event);
		return JNI_FALSE;
	}

	WLog_INFO(TAG, "[JNI-C] event pushed OK");
	return JNI_TRUE;
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	libFreeRDPClass = (*env)->FindClass(env, JAVA_LIBFREERDP_CLASS);
	...
	return init_callback_environment(vm, env, libFreeRDPClass);
}
...
void JNICALL JNI_OnUnload(JavaVM* vm, void* reserved)
{
	...
	deinit_callback_environment(env);
}

JNIEXPORT jint JNICALL
Java_com_freerdp_freerdpcore_services_LibFreeRDP_setClientDecoding(
    JNIEnv* env, jclass cls, jlong instance, jboolean enable)
{
	freerdp* inst = (freerdp*)(uintptr_t)instance;
	if (!inst || !inst->context)
		return -1;
	...
	// Set DeactivateClientDecoding to opposite of enable
	// (when enable=false, we want to deactivate decoding)
	BOOL deactivate = enable ? FALSE : TRUE;
	if (!freerdp_settings_set_bool(settings, FreeRDP_DeactivateClientDecoding, deactivate))
	{
		WLog_ERR(TAG, "Failed to set DeactivateClientDecoding");
		return -4;
	}

	// Enable SuppressOutput capability
	if (!freerdp_settings_set_bool(settings, FreeRDP_SuppressOutput, TRUE))
	{
		WLog_ERR(TAG, "Failed to enable SuppressOutput capability");
		return -5;
	}

	// Send SuppressOutput PDU to server
	// allowDisplayUpdates: TRUE = resume graphics, FALSE = suppress graphics
	BOOL allowDisplayUpdates = enable ? TRUE : FALSE;
	
	// Define full desktop area
	RECTANGLE_16 rect = { 0 };
	rect.left = 0;
	rect.top = 0;
	...
	if (update->SuppressOutput)
	{
		...
		{
			WLog_ERR(TAG, "SuppressOutput PDU failed");
			return -6;
		}
		
		WLog_INFO(TAG, "Client decoding %s, SuppressOutput sent (allowDisplayUpdates=%d)",
		          enable ? "enabled" : "disabled", allowDisplayUpdates);
		return 0;
	}
	else
	{
		WLog_WARN(TAG, "SuppressOutput callback not available");
		return -7;
	}
}
    ...
    jint delay, jint interval, jint retries)
{
	...
	if (!inst)
	{
		WLog_ERR(TAG, "freerdp_set_tcp_keepalive: Invalid instance (NULL)");
		return JNI_FALSE;
	}
	
	if (!inst->context)
	{
		WLog_ERR(TAG, "freerdp_set_tcp_keepalive: Invalid context (NULL)");
		return JNI_FALSE;
	}
	...
	if (!settings)
	{
		WLog_ERR(TAG, "freerdp_set_tcp_keepalive: Invalid settings (NULL)");
		return JNI_FALSE;
	}
	...
	{
		WLog_ERR(TAG, "Failed to set TcpKeepAlive=%d", enabled);
		return JNI_FALSE;
	}
	...
	if (enabled)
	{
		...
		{
			WLog_ERR(TAG, "Failed to set TcpKeepAliveDelay=%d", delay);
			return JNI_FALSE;
		}
		
		if (!freerdp_settings_set_uint32(settings, FreeRDP_TcpKeepAliveInterval, (UINT32)interval))
		{
			WLog_ERR(TAG, "Failed to set TcpKeepAliveInterval=%d", interval);
			return JNI_FALSE;
		}
		
		if (!freerdp_settings_set_uint32(settings, FreeRDP_TcpKeepAliveRetries, (UINT32)retries))
		{
			WLog_ERR(TAG, "Failed to set TcpKeepAliveRetries=%d", retries);
			return JNI_FALSE;
		}
		
		WLog_INFO(TAG, "✓ TCP Keepalive enabled: delay=%ds, interval=%ds, retries=%d", 
		          delay, interval, retries);
	}
	else
	{
		WLog_INFO(TAG, "TCP Keepalive disabled");
	}
...
/**
 * 发送同步事件作为轻量级心跳
 * 用于替代鼠标微动，实现更高效的RDP会话保活
 * 
 * @param env JNI环境
 * @param cls Java类引用
 * @param instance FreeRDP实例指针
 * @param flags 键盘锁定状态标志位 (0=所有锁定键关闭，适合作为纯心跳使用)
 * @return JNI_TRUE=成功, JNI_FALSE=失败
 */
JNIEXPORT jboolean JNICALL
    ...
    JNIEnv* env, jclass cls, jlong instance, jint flags)
{
	...
	if (!inst)
	{
		WLog_ERR(TAG, "freerdp_send_synchronize_event: Invalid instance (NULL)");
		return JNI_FALSE;
	}
	
	if (!inst->context)
	{
		WLog_ERR(TAG, "freerdp_send_synchronize_event: Invalid context (NULL)");
		return JNI_FALSE;
	}
	...
	if (!input)
	{
		WLog_ERR(TAG, "freerdp_send_synchronize_event: Invalid input (NULL)");
		return JNI_FALSE;
	}
	
	// 发送同步事件到RDP服务器
	// 这是一个轻量级的RDP协议事件，仅约8字节，用于保持会话活跃
	BOOL result = freerdp_input_send_synchronize_event(input, (UINT32)flags);
	
	if (!result)
	{
		WLog_WARN(TAG, "freerdp_send_synchronize_event: Failed to send synchronize event");
	}
